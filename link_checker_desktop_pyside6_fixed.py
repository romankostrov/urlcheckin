...
class Switch(QCheckBox):
    def __init__(self, text="", parent=None):
        ...
        self.setStyleSheet(f"""
            ...
            QCheckBox::indicator {{
                width: {self.track_w}px;
                height: {self.track_h}px;
            }}
            QCheckBox::indicator {{
                border: none;
                background: transparent;
    ...
    def paintEvent(self, event):
        ...
        y = max(0, (self.height() - self.track_h) // 2)
        rect_x = 0

        track_color = QColor("#111111") if self.isChecked() else QColor("#e6e8ec")
        p.setPen(Qt.NoPen)
        p.setBrush(track_color)
        p.drawRoundedRect(rect_x, y, self.track_w, self.track_h, self.track_h / 2, self.track_h / 2)

        knob_x = rect_x + (self.track_w - self.knob_d - 3 if self.isChecked() else 3)
        knob_y = y + 3
        p.setBrush(QColor("#ffffff"))
        p.drawEllipse(knob_x, knob_y, self.knob_d, self.knob_d)
        p.end()
